...
assign we = (pkt_state == pkt_data) && rdy && !(overflow_stop_en && full);  //[Modified]
assign load_data = ((pkt_state == pkt_idle) && request) || ((pkt_state == pkt_data) && request);
assign inc_addr = ((pkt_state == pkt_idle) && request) ||  ((pkt_state == pkt_data) && (rdy || request) && !(overflow_stop_en && full)); //[Modified]
