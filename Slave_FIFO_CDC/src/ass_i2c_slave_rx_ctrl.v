...
always @(posedge clk or negedge rstb) begin
    if (!rstb) begin
        ...
        init_r <= {init_r[0],init};        
    end else if (init_ack) begin
        init_r <= 2'b00;       
    end
end

assign init_req = init_r[1];


assign request = read_req || seq_read_req;
reg [1:0] request_r;
always @(posedge clk or negedge rstb) begin
    if (!rstb) begin
        request_r <= 2'b00;
    end else if (!request_req) begin
        request_r <= {request_r[0],request};
    end else if (request_ack) begin
        request_r <= 2'b00;
    end
end
assign request_req = request_r[1];
...
//-----------output logic---------------------------------------------
wire rdy        = rx_fall && rx_done && write_valid; //write
